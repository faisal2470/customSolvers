...
License
    ...
    - second attempt to implement the velocity correction
...
) const
{
    ...
    if (DTFuncs_.size())
    {
        ...
        tmpDivj.ref() += -fvc::div(jCorr*fvc::interpolate(Yi)*Yi.mesh().magSf()); 
        return tmpDivj;
    }
    else
    {
        ...
        tmpDivj.ref() += -fvc::div(jCorr*fvc::interpolate(Yi)*Yi.mesh().magSf());
        Info << "Entered correction velocity code properly" << endl;
        //Info << "The maximum value of the correction velocity is: " << max(-jCorr/this->momentumTransport().rho())  << endl;
        return tmpDivj;
    }
}
