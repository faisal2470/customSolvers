...
License
    ...
    Taken from the github repo, modified in three points:
    - corrected definition of Dm so that 1-Yi is done correctly at the
      numerator
    - corrected the polynomial formulation of Dij as it is done in chem1d,
      so that the trot coefficients can be copied directly
    - first attempt to implement the velocity correction in j and divj.
      Compiles, but never enters the if because the default specie is
      not solved in reacingFoam, so no difference in the results.
...
) const
{
    ...
    //Info << "The value of tmpDivq is: " << tmpDivq << endl;
    return tmpDivq;
}
...
) const
{
    ...
    forAll(Y, i)
    {
        if (&Y[i] == &Yi)  // Compare the addresses of Yi and the species field
        {
            YiIndex = i;
            break;
        }
    ...
    if (DTFuncs_.size())
    {
        ...
        if (YiIndex == composition.defaultSpecie())
        {
            //const PtrList<volScalarField>& Y = composition.Y();
            if (Y.size())
            {
    ...
    else
    {
        if (YiIndex == composition.defaultSpecie())
        {
            ...
            if (Y.size())
            {
                ...
                tmpj.ref() = -sumJ;
                Info << "The value of tmpj inside N2 is: " << tmpj << endl;
            }
        }
    }
    return tmpj;
...
/*             surfaceScalarField sumJh
            (
                ...
                (
                    "sumJh",
                    Y[0].mesh(),
                    dimensionedScalar(sumJ.dimensions(), 0)
                )
            ); */

            /* const volScalarField sumJcorr
            (
                IOobject
                (
                    "sumJcorr", // Name of the field
                    this->momentumTransport().mesh().time().timeName(), // Time
                    this->momentumTransport().mesh(), // Mesh
                    IOobject::NO_READ, // No read from file
                    IOobject::NO_WRITE, // No write to file
                    false // No dynamic allocation
                ),
                this->momentumTransport().mesh(), // Mesh reference
                dimensionedScalar("one", dimless, 1.0) // Uniform value of one
            ); */
            ...
            forAll(Y, i)
            {
                if (i != composition.defaultSpecie())
                {
                    const surfaceScalarField ji(this->j(Y[i]));
                    sumJ += ji;
                    // sumJh += ji*fvc::interpolate(sumJcorr);
                }
            }
            tmpDivj.ref() += -fvm::Sp(fvc::div(-sumJ), Yi); 
            //tmpDivj.ref() += -fvm::Sp(fvc::div(-sumJh), Yi); 
        }
    }
    else
    {
...
/*             surfaceScalarField sumJh
            ...
            tmpDivj.ref() += -fvm::Sp(fvc::div(-sumJ), Yi); 
            //tmpDivj.ref() += -fvm::Sp(fvc::div(-sumJh), Yi); 
            Info << "The value of tmpDivj inside N2 is: " << tmpDivj << endl;
        }
    }
    ...
    forAll(Dm_, i)
    {
        ...
        (
            ...
            (
                1/Wm
                - Y[i]/Wm
                //- Y[i]/dimensionedScalar("Wi", Wm.dimensions(), composition.Wi(i))
